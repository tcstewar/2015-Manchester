...
with model:
    ...
    def gate_food(scare):
        return -10 if scare > 0.5 else 0
    nengo.Connection(scare, do_food.neurons, transform=[[1]]*do_food.n_neurons,
                     function=gate_food)    

    # A system for figuring out if it should run home
    do_home = nengo.Ensemble(n_neurons=300, dimensions=2)
    nengo.Connection(position, do_home, transform=-5)
    nengo.Connection(do_home, movement)
    
    def gate_home(scare):
        return -50 if scare < 0.5 else 0
    nengo.Connection(scare, do_home.neurons, transform=[[1]]*do_home.n_neurons,
                     function=gate_home)    
